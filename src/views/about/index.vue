  ...
  <div class="about-page">
    ...
    <section class="story-grid">
      <article class="mission">
        ...
        <p>
          上传后自动读取应用名称、图标、安卓包名和版本号。同一包名持续追加版本，新包名自动建立应用档案，并保留文件摘要、版本历史和下载入口。
        </p>
    ...
    <section class="architecture">
      <header>
        <p class="section-label">LIVE ARCHITECTURE</p>
        <h3>当前线上架构</h3>
      </header>
      <div class="service-list">
        <article v-for="item in services" :key="item.no">
          <b>{{ item.no }}</b>
          <span>{{ item.name }}</span>
